...
<div class="JavaScript-ItRE30f-text"><strong>字符</strong>是计算机软件处理文字时最基本的单位，可能是字母，数字，标点符号，空格，换行符，汉字等等。<strong>字符串</strong>是0个或更多个字符的序列。<strong>文本</strong>也就是文字，字符串。说某个字符串<strong>匹配</strong>某个正则表达式，通常是指这个字符串里有一部分（或几部分分别）能满足表达式给出的条件。</div>
...
　　`0\d{2}-\d{8}|0\d{3}-\d{7}` 这个表达式能**匹配两种以连字号分隔的电话号码：一种是三位区号，8位本地号(如 `010-12345678`)，一种是4位区号，7位本地号(如 `0376-2233445`)**。
...
　　`(?<=exp)` 也叫**零宽度正回顾后发断言**，它**断言自身出现的位置的前面能匹配表达式exp**。比如 `(?<=\bre)\w+\b`会匹配以 re 开头的单词的后半部分(除了re以外的部分)，例如在查找 `reading a book` 时，它匹配ading。
...
<p class="JavaScript-ItRE30f-thead">表7.尚未详细讨论的语法</p>
<div class="JavaScript-ItRE30f-table">
    <table style="margin-top: 0;">
        <thead>
            <tr><th>代码/语法</th><th>说明</th></tr>
...
<script>
!function (){
    let ItRE30f = document.querySelectorAll('.js-ItRE30f-y')
    /* 清除格式，点击隐藏/显示字体加粗 */
    ItRE30f[0].onclick = function(){
        textBlodAddClass()
        let content = ItRE30f[0].innerText
        if(content==='清除格式') {
            ItRE30f[0].innerText = '显示格式'
        } else {
            ItRE30f[0].innerText = '清除格式'
        }
    }
    /* 隐藏边注点击事件，点击隐藏/显示边注 */
    ItRE30f[1].onclick = function(){
        textsElementAddClass()
        let content = ItRE30f[1].innerText
        if(content==='隐藏边注') {
            ItRE30f[1].innerText = '显示边注'
        } else {
            ItRE30f[1].innerText = '隐藏边注'
        }
    }
    /* 清除格式 */
    function textBlodAddClass(){
        let text = document.querySelectorAll('article.markdown strong')
    ...
    /* 隐藏边注 */
    function textsElementAddClass(){
        let text = document.querySelectorAll('.JavaScript-ItRE30f-text')
        for(let i=0; i<text.length; i++){
            const item = text[i]
            item.classList.toggle('active')
        }
